...
void capNhatMonAnTuMang(MonAn menu[], int* soMonMang, char duongDan[], int* soMonFile) {
    ...
    for(int i = 0; i < *soMonMang; i++) {
        ...
        for(int j = 0; j < *soMonFile; j++) {
            if(strcmp(menuFile[j].tenMon, menu[i].tenMon) == 0) {
        ...
        if (coMat == 0) {
            saoChepMonAn(&menuMoi[*soMonFile], &menu[i]);
            (*soMonFile)++;
        }
    }

    resetFile(LUA_CHON_MENU);
    themNhieuMonAnVaoFile(menuMoi, *soMonFile, duongDan);
...
void capNhatKhoTuMang(NguyenLieu kho[], int* soLuongKho, char duongDan[], int* soLuongFile) {
    ...
    for(int i = 0; i < *soLuongKho; i++) {
        coMat = 0;
        if(*soLuongFile >= MAX_NGUYEN_LIEU) {
            printf("File da day\n");
            break;
        }

        for(int j = 0; j < *soLuongFile; j++) {
            if(strcmp(khoFile[j].tenNguyenLieu, kho[i].tenNguyenLieu) == 0) {
        ...
        if (coMat == 0) {
            saoChepNguyenLieu(&khoMoi[*soLuongFile], &kho[i]);
            (*soLuongFile)++;
        }
    }

    resetFile(LUA_CHON_KHO);
    themNhieuNguyenLieuVaoFile(khoMoi, *soLuongFile, duongDan);
